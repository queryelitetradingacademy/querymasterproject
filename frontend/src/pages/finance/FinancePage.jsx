...
export default function FinancePage() {
  ...
  useEffect(() => {
    if (tab === 'transactions') fetchData();
  }, [tab, book, typeFilter, fy, search, page]);
  useEffect(() => {
    fetchSummary();
  }, [book, fy]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      downloadBlob(
        res.data,
        `Finance_${now.getMonth() + 1}_${now.getFullYear()}.xlsx`,
      );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">
            Track income, expenses, investments and tax
          </p>
        </div>
                    ...
                    <tbody>
                      {transactions.map((tx) => (
                        <tr key={tx._id}>
                          ...
                          <td>
                            <span
                              className={`badge ${bookColor[tx.book]} text-xs`}
                            >
                              {bookLabel[tx.book]}
                            </span>
                          </td>
                            ...
                            <div className="flex gap-1">
                              {can('finance', 'edit') && (
                                <button
                                  ...
                                  className="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded text-blue-500 text-sm"
                                >
                                  ✏️
                                </button>
                              )}
